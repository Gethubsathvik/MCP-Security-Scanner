...
class SecretExposureCheck(Check):
    ...
    _patterns = (
        ...
        re.compile(r"\b(?:api[_ -]?key|secret|token|password|credential)\s*[:=]\s*[\"']?[\w/+.-]{12,}", re.I),
    ...
    def run(self, manifest: MCPManifest) -> list[Finding]:
        ...
        for category, item in manifest_items(manifest):
            ...
            match = next((pattern.search(searchable) for pattern in self._patterns if pattern.search(searchable)), None)
            if match:
                findings.append(
                    Finding(
