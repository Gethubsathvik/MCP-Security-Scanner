...
async def _collect(method: Callable[..., Awaitable[Any]]) -> list[dict[str, Any]]:
    ...
    while True:
        ...
        values = (
            getattr(result, "tools", None)
            or getattr(result, "resources", None)
            or getattr(result, "prompts", None)
            or []
        )
...
class MCPClient:
    ...
    async def _connection(self) -> AsyncGenerator[tuple[Any, Any], None]:
        if self.transport == "stdio":
            ...
            params = StdioServerParameters(
                command=self.command, args=self.args, cwd=self.cwd, env=merged_env
            )
    ...
    async def introspect(self) -> MCPManifest:
        async with self._connection() as (read_stream, write_stream):
            try:
                return await _introspect(
                    read_stream, write_stream, self.target, self.transport, self.remote_signals
                )
                ...
                async with sse_client(self.target, headers=self.headers or {}) as streams:
                    return await _introspect(
                        streams[0], streams[1], self.target, self.transport, self.remote_signals
                    )
